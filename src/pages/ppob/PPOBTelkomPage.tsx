...
// Telkom Service Types
const telkomServices = [
  { id: "indihome", name: "IndiHome", icon: "wifi", color: "bg-red-100", iconColor: "text-red-500" },
  { id: "telepon", name: "Telepon Rumah", icon: "phone", color: "bg-blue-100", iconColor: "text-blue-500" }
];

// Recent payments
const recentPayments = [
  { id: "1", name: "Rumah Utama", number: "021345678", type: "indihome", period: "Juni 2025" },
  { id: "2", name: "Kantor", number: "022987654", type: "telepon", period: "Mei 2025" },
];

const PPOBTelkomPage = () => {
  const navigate = useNavigate();
  const [serviceType, setServiceType] = useState("indihome");
  ...
  const checkCustomerNumber = () => {
    ...
    setTimeout(() => {
      setCustomerInfo({
        name: "Joko Widodo",
        address: "Jl. Soekarno Hatta No. 17, Menteng, Jakarta Pusat",
        accountNumber: serviceType.toUpperCase() + "-" + customerNumber,
        period: "Juni 2025",
        dueAmount: serviceType === "indihome" ? 385000 : 125000,
        dueDate: "20 Juni 2025",
        packageInfo: serviceType === "indihome" ? "Internet 50Mbps + TV + Telepon" : "Telepon Rumah"
      });
      setIsChecking(false);
    }, 1000);
  };
  ...
  const handlePayment = () => {
    ...
    // Simulate API call
    setTimeout(() => {
      const txId = `TELKOM-${Date.now().toString().slice(-8)}`;
    ...
    <MobileLayout>
      ...
      <div className="bg-white sticky top-0 z-10 border-b border-gray-100">
        <div className="flex items-center px-4 py-3">
          ...
          <div className="ml-4">
            <h1 className="text-lg font-semibold">Telkom</h1>
            <p className="text-xs text-muted-foreground">Bayar tagihan Telkom IndiHome & Telepon</p>
          </div>
        </div>
      </div>
          ...
          {telkomServices.map((service) => (
            ...
            >
              <div className={`h-10 w-10 rounded-full ${service.color} flex items-center justify-center`}>
                {service.id === "indihome" ? (
                  <Icons.wifi className={`h-5 w-5 ${service.iconColor}`} />
                ) : (
                  <Icons.phone className={`h-5 w-5 ${service.iconColor}`} />
                )}
              </div>
              <div>
                <h3 className="font-medium">{service.name}</h3>
                <p className="text-xs text-muted-foreground">
                  {service.id === "indihome" ? "Internet, TV & Telepon" : "Telepon Rumah"}
                </p>
              </div>
            </div>
          ))}
        ...
        <Card>
          <CardContent className="p-4">
            <Label className="text-base font-medium mb-3 block">
              {serviceType === "indihome" 
                ? "Nomor IndiHome / Internet ID" 
                : "Nomor Telepon Rumah"}
            </Label>
              ...
              <div>
                <div className="relative">
                  <Input
                    ...
                    placeholder={serviceType === "indihome" ? "Nomor Internet ID" : "Nomor Telepon"}
                  />
                  <div className="absolute left-3 top-1/2 transform -translate-y-1/2">
                    {serviceType === "indihome" ? (
                      <Icons.wifi className="h-5 w-5 text-gray-400" />
                    ) : (
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  {serviceType === "indihome" 
                    ? "Nomor Internet ID tertera pada tagihan IndiHome Anda" 
                    : "Nomor telepon tertera pada tagihan telepon Anda"}
                </p>
              </div>

              {/* Recent payments */}
              {recentPayments.filter(p => p.type === serviceType).length > 0 && (
                <div>
                  ...
                  <div className="space-y-2">
                    {recentPayments
                      .filter(payment => payment.type === serviceType)
                      .map((payment) => (
                        <div
                          key={payment.id}
          ...
          <Card>
            <CardContent className="p-4">
              <div className="space-y-4">
                ...
                <div>
                  <Label className="text-sm text-muted-foreground">Nomor Pelanggan</Label>
                  <div className="font-medium">{customerInfo.accountNumber}</div>
                </div>
        ...
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription className="text-center">
              Tagihan {serviceType === "indihome" ? "IndiHome" : "Telepon Rumah"} Anda telah berhasil dibayar
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="bg-muted/50 p-4 rounded-lg mb-4">
            <div className="space-y-2">
              <div className="flex justify-between">
                ...
                <span className="font-semibold">{customerInfo?.accountNumber}</span>
              </div>
              <div className="flex justify-between">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-semibold">
                  {serviceType === "indihome" ? "IndiHome" : "Telepon Rumah"}
                </span>
              </div>
              <div className="flex justify-between">
